...
int main() {
  if (remap_oracle::kCases.size() != 14 || remap_oracle::kMutations.size() != 7 || remap_oracle::kBindingNames.size() != 303U) return 1;
  ...
  (void)c.controls.zones[0].bounds;
  ...
  (void)m.name; (void)m.anchor; (void)m.replacement; (void)m.source_sha256; (void)m.mutated_module_text_sha256;
