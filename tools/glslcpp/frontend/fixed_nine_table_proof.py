"""Exact proof for the pinned, fixed-size 3x3 convolution tables."""
...
DISTORTION_KEY = "mixer/distortion:distortion"
SOURCE_LOCKS = {
    SHARPEN_KEY: (
    ...
    LIGHTING_KEY: (
        "a0601f7012f385c14c1bdb9f462e5dcb303fe05cfbb4645484d5d1bd629e1a4f",
        "c35208b3f864c1a3a75e0aa1f500fab3391c3bef31c6e80da153f04e02b6f343",
    ),
    DISTORTION_KEY: (
        "569fbab57b57baad275a60facfd70b913afe76d69a724b682e821883d40dcae8",
        "5b484e6009b3dd64a8f6c19551dd83f09cab2cf707432ba53cc2ec7ed4bd14db",
    ),
}

# The pinned host function carrying the fixed-nine tables. Every existing
# member of this cluster (sharpen, sobel) declares its tables directly in
# `main()`. Lighting's Sobel-shaped tables live one level down, inside the
# `calculateNormal` helper it calls unconditionally -- the table shape, loop
# shape, and read pattern are otherwise byte-identical to sobel's, so this
# widens ONLY the host lookup (never the proof's structural requirements)
# to name the exact host function per program instead of assuming "main".
_HOST_NAME = {LIGHTING_KEY: "calculateNormal", DISTORTION_KEY: "calculateNormal"}

_PROFILES = {
    SHARPEN_KEY: (("kernel", "float", 6), ("offsets", "vec2", 16)),
    SOBEL_KEY: (("sobel_x", "float", 6), ("sobel_y", "float", 16),
                ("offsets", "vec2", 26)),
    LIGHTING_KEY: (("sobel_x", "float", 1), ("sobel_y", "float", 11),
                   ("offsets", "vec2", 21)),
    DISTORTION_KEY: (("sobel_x", "float", 1), ("sobel_y", "float", 11),
                     ("offsets", "vec2", 21)),
}
_PINNED_ARRAY_SYMBOL_IDS = {
    DISTORTION_KEY: (78, 79, 80),
}
_LOOP_INDEX = {SHARPEN_KEY: 27, SOBEL_KEY: 38, LIGHTING_KEY: 33,
               DISTORTION_KEY: 33}
_BODY_COUNT = {SHARPEN_KEY: 29, SOBEL_KEY: 43, LIGHTING_KEY: 38,
               DISTORTION_KEY: 39}
_LOOP_BODY_COUNT = {DISTORTION_KEY: 4}
_PAYLOAD = {SHARPEN_KEY: 144, SOBEL_KEY: 216, LIGHTING_KEY: 216,
            DISTORTION_KEY: 216}
_LOOP_READS = {SHARPEN_KEY: ("offsets", "kernel"),
               SOBEL_KEY: ("offsets", "sobel_x", "sobel_y"),
               LIGHTING_KEY: ("offsets", "sobel_x", "sobel_y"),
               DISTORTION_KEY: ("offsets", "sobel_x", "sobel_y")}
_TYPED_IR_LOCKS = {
    ...
    LIGHTING_KEY: "e5a7a72859a571065a6d6a5660ec6cf2181ce0f4256f1edc028da3cfb47f9240",
    DISTORTION_KEY: "00643dc5cfdd4bea437404400899f77164d4a2583e0e2b1d5653b224c262ff40",
}
_WHOLE_PROGRAM_LOCKS = {
    ...
    LIGHTING_KEY: "a24152ae1a234052831e0ab0761aa1b5389ed6aa9a3ea59b5ae1a09216c6220b",
    DISTORTION_KEY: "54a54c4fe1753ffb337e13edfadd3d1152f0c5e10b4f39ffb22f6de8929762e9",
}
...
def prove_fixed_nine_local_tables(program: TypedProgram) -> FixedNineLocalTableProof | None:
    ...
    if whole_program_fingerprint != _WHOLE_PROGRAM_LOCKS[key]:
        return None
    host_name = _HOST_NAME.get(key, "main")
    mains = [function for function in functions if function.name == host_name and function.body]
    if len(mains) != 1:
        return None
    main = mains[0]
    ...
    for role, element, declaration_index in _PROFILES[key]:
        ...
        if (declaration.kind != "declaration" or declaration.symbol is None
            ...
            return None
        pinned_ids = _PINNED_ARRAY_SYMBOL_IDS.get(key)
        if (pinned_ids is not None
                and declaration.symbol_id != pinned_ids[len(arrays)]):
            return None
    ...
    expected_loop_body_count = _LOOP_BODY_COUNT.get(key, 2 if key == SHARPEN_KEY else 3)
