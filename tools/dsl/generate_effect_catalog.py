...
COMPATIBILITY_SHA256 = "592137006a16b4b3a650118fc723756cfdc0f4a394514deee0b02cecbe30e918"
...
def _emit_cpp(records: list[dict[str, Any]], compatibility: dict[str, Any], authority: dict[str, Any], normalized_hash: str, payload_hash: str) -> bytes:
    ...
    lines += ["const EffectDefinition* EffectCatalog::find(const std::string& id) const {", "  const auto found = index.find(id);", "  if (found != index.end()) return &definitions[found->second];", "  for (std::size_t i = 0; i < definitions.size(); ++i) {", "    if (definitions[i].id == id) { index.emplace(id, i); return &definitions[i]; }", "  }", "  return nullptr;", "}", "", "const EffectCatalog& effect_catalog() {", "  static const EffectCatalog catalog = [] {", "    EffectCatalog c(EffectCatalog::ProductionToken{});"]
