...
def _emit_cpp(records: list[dict[str, Any]], compatibility: dict[str, Any], authority: dict[str, Any], normalized_hash: str, payload_hash: str) -> bytes:
    ...
    for reference in compatibility["reference_passes"]:
        lines += ["    {", "      ReferencePassCompatibility p;", f"      p.effect_id = {_cpp_string(reference['effect_id'])};", f"      p.pass_index = {reference['pass_index']};", f"      p.pass_name = {_cpp_string(reference['pass_name'])};", f"      p.program_key = {_cpp_string(reference['program_key'])};", f"      p.status = {_cpp_string(reference['status'])};", "      p.reasons = {" + ", ".join("{" + _cpp_string(str(r.get('code',''))) + ", " + _cpp_string(str(r.get('detail',''))) + "}" for r in reference.get('reasons', [])) + "};", "      c.reference_passes.push_back(std::move(p));", "    }"]
