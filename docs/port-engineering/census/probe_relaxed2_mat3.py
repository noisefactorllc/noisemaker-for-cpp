...
from tools.glslcpp import generate_typed_slice_relaxed2 as genr  # noqa: E402
...
def main() -> int:
    ...
    for key in KEYS:
        ...
        try:
            ...
            row["after_mat3_admission"] = "VALIDATOR-PASS"
            try:
            ...
            row["after_mat3_admission"] = first_line(error)
        results.append(row)
    out = pathlib.Path("docs/port-engineering/census/relaxed2_mat3_probe.json")
    out.write_text(json.dumps(results, indent=1, sort_keys=True))
    for r in results:
        print(r["key"], "->", r.get("after_mat3_admission"), "| emit:", r.get("emitter"))
    return 0
