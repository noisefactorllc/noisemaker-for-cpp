...
run() {
    ...
    printf '%s\n' "$out" > "$SDD/task-29-gate-native-${label// /-}.log"
}
...
lane "debug" "build-task29-final-debug" -DCMAKE_BUILD_TYPE=Debug
lane "release" "build-task29-final-release" -DCMAKE_BUILD_TYPE=Release
...
SAN_DIR=build-task29-final-sanitize
rm -rf "$SAN_DIR"
...
ASAN_OPTIONS=detect_leaks=1 UBSAN_OPTIONS=print_stacktrace=1:halt_on_error=1 \
    "./$SAN_DIR/noisemaker-cpu-tests" > "$SDD/task-29-sanitize-attempt1.log" 2>&1
ATTEMPT1=$?
printf 'attempt 1 exit=%d\n' "$ATTEMPT1"
tail -5 "$SDD/task-29-sanitize-attempt1.log"
...
if [ $ATTEMPT1 -eq 0 ]; then
    ...
    ASAN_OPTIONS=detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1:halt_on_error=1 \
        "./$SAN_DIR/noisemaker-cpu-tests" > "$SDD/task-29-sanitize-attempt2.log" 2>&1
    ATTEMPT2=$?
    printf 'attempt 2 exit=%d\n' "$ATTEMPT2"
    tail -5 "$SDD/task-29-sanitize-attempt2.log"
