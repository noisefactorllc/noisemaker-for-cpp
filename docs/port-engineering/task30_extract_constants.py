...
print()
print("=== bvec-typed nodes anywhere ===")
for f in program.functions:
    for statement in f.body:
        for item, parent, epath, chain in walk_stmt(statement):
            d = item.type.display() if item.type is not None else None
            if d and d.startswith("bvec"):
                print(f"  fn={f.id} kind={item.kind} callee={getattr(item,'callee',None)} "
                      f"type={d} span={span(item)}")
