...
[[nodiscard]] inline float cos(double value) { return noisemaker::f32(std::cos(value)); }
[[nodiscard]] inline float exp(double value) { return noisemaker::f32(std::exp(value)); }
...
[[nodiscard]] inline float sin(double value) { return noisemaker::f32(std::sin(value)); }
...
template <std::size_t N> requires(N == 3) [[nodiscard]] inline Vec<N,float> tanh(const Vec<N,float>& value) { return detail::map_float(value,[](double lane){return std::tanh(lane);}); }
...
template <std::size_t N> [[nodiscard]] inline Vec<N,float> reflect(const Vec<N,float>& incident,const Vec<N,float>& normal) { const double scale=2.0*dot(normal,incident);Vec<N,float> result;for(std::size_t i=0;i<N;++i)result[i]=noisemaker::f32(static_cast<double>(incident[i])-scale*normal[i]);return result; }
...
using UniformValue=std::variant<float,double,std::int32_t,std::uint32_t,bool,Vec2,Vec3,Vec4,IVec2,IVec3,IVec4,UVec2,UVec3,UVec4,BVec2,BVec3,BVec4,Mat2,Mat3,Mat4>;
